...
import "./CharlasRealizadas.css";
import ApiService from "../../api/ApiService";

export default class CharlasRealizadasComponent extends Component {
  constructor() {
    super();
    this.state = {
      charlasRealizadas: [],
  ...
  async fetchCharlasRealizadas() {
    try {
      ...
      const charlasRealizadas = charlas.filter(
        (charla) =>
          charla.estadoCharla === "ACREDITADA" ||
          charla.estadoCharla === "COMPLETADA"
      );
      this.setState({ charlasRealizadas, isLoading: false });
    } catch (error) {
      console.error("Error al obtener las charlas:", error);
      this.setState({ isLoading: false });
    }
    ...
    return (
      <div className="container-charlas-activas">
        ...
        {this.state.isLoading ? (
          <Loader />
        ) : this.state.charlasRealizadas.length > 0 ? (
          <div className="charlas-container">
            {this.state.charlasRealizadas.map((charla) => (
